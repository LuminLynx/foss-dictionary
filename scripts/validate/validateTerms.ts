...
const CORPORATE_BUZZWORDS = [
  'leverage', 'synergy', 'paradigm', 'enterprise-grade', 'scalable solution',
  'best-of-breed', 'world-class', 'industry-leading', 'cutting-edge',
...
function checkTone(definition: string, note?: string): ValidationResult[] {
  ...
  if (/^(A|An)\s/.test(definition)) {
    issues.push({
      level: 'warning',
      field: 'definition',
      message: 'Definition starts with "A/An" - consider a more conversational tone',
...
function validateTerm(term: any, allTermIds: Set<string>, allTermNames: string[]): ValidationResult[] {
  ...
  if (!term.id || typeof term.id !== 'string') {
    results.push({ level: 'error', field: 'id', message: 'Missing or invalid ID field' });
  }
  
  if (!term.term || typeof term.term !== 'string') {
    results.push({ level: 'error', field: 'term', message: 'Missing or invalid term field' });
  }
...
function writeReport(report: ValidationReport, outputDir: string = './validation-output') {
  ...
  const markdownPath = path.join(outputDir, `validation-report-${timestamp}.md`);
...
async function runValidation() {
  ...
  const schemaResult = await validate("https://example.com/terms-schema", terms);
  ...
  for (const term of terms) {
    if (!term || !term.id || !term.term) {
      console.warn(yellow(`⚠️ Skipping malformed term: ${JSON.stringify(term)}`));
      continue;
    }
